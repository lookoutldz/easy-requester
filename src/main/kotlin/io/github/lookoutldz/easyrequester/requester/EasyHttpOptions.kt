 ...
 *  OPTIONS请求核心类
 */
class EasyHttpOptions<T> private constructor(
    url: String,
    ...
    companion object {
        ...
        inline fun <reified T> doRequest(
            ...
            noinline successHandler: ((T?) -> Unit)? = null
        ) {
    ...
    class Builder<T> : AbstractEasyHttp.Builder<T> {
        ...
        override fun build(): EasyHttpOptions<T> {
            return EasyHttpOptions(
                url = url,
                params = params,
                headers = headers,
                cookies = cookies,
    ...
    override fun execute() {
        val request = commonRequestGenerator(url, params, headers, cookies)
            .method("OPTIONS", null)
            .build()
